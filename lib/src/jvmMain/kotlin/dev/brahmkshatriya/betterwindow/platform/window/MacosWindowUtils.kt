...
internal class MacosWindowUtils : AwtWindowUtils() {
    ...
    interface Quartz : Library {
        ...
        companion object {
            val INSTANCE: Quartz =
                Native.load(if (Platform.isMac()) "Quartz" else "c", Quartz::class.java)
        }
    }
    ...
    override fun setPreventScreenSaver(prevent: Boolean) = preventScreenSaverLock.withLock {
        if (prevent) {
            if (screenSaverProcess == null) {
                println("Launching caffeinate to prevent screen saver")
                try {
                    ...
                    println("Failed to launch caffeinate, see cause")
                    e.printStackTrace()
                }
            } else {
                return
            ...
            println("Stopping caffeinate")
            try {
                screenSaverProcess?.destroy()
            } catch (e: Exception) {
                println("Failed to stop caffeinate, see cause")
                e.printStackTrace()
            }
